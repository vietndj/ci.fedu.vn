...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const searchParams = new URL(rawUrl, 'https://ci.fedu.vn').searchParams;
    const hubNested = (req.query?.hub as any) || {};

    const mode = req.query['hub.mode'] || hubNested.mode || searchParams.get('hub.mode') || req.query['mode'];
    const token = req.query['hub.verify_token'] || hubNested.verify_token || searchParams.get('hub.verify_token') || req.query['token'];
    const challenge = req.query['hub.challenge'] || hubNested.challenge || searchParams.get('hub.challenge') || req.query['challenge'];
